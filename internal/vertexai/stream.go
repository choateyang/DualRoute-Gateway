...
import (
	...
	"dualroute-gateway/internal/spool"
...
// StreamChat races one streaming generation across all candidate exits; the
// first candidate producing data wins and streams to completion.
func (c *VertexAIClient) StreamChat(ctx context.Context, exits []string, model string, geminiPayload map[string]any, yield func(StreamChunk) bool, opts ...RaceOption[<-chan StreamChunk]) {
	...
	streamParallelURIs(routedCtx, c.cfg, exits, op, yield, opts...)
}
