...
func (c *VertexAIClient) CompleteChatImage(ctx context.Context, exits []string, model string, geminiPayload map[string]any) ([]ImageData, error) {
	result, err := c.CompleteChat(ctx, exits, model, geminiPayload)
	if err != nil {
		return nil, err
	}
...
func (c *VertexAIClient) CompleteChatAudio(ctx context.Context, exits []string, model string, geminiPayload map[string]any) (AudioData, error) {
	result, err := c.CompleteChat(ctx, exits, model, geminiPayload)
	if err != nil {
